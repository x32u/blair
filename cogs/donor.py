...
class Donor(commands.Cog):
    ...
    async def chatgpt(self, ctx, *, prompt: str):
        ...
        emb = discord.Embed(description=f"> {ctx.author.mention}: **__Please wait a moment.__**")
    ...
    async def selfprefix_remove(self, ctx):
        ...
        await self.client.pool.execute(
            "DELETE FROM prefixes WHERE user_id = $1;",
            ctx.author.id)
    ...
    async def selfprefix_show(self, ctx):
        ...
        result = await self.client.pool.fetchrow(
            "SELECT prefix FROM prefixes WHERE user_id = $1;", 
            ctx.author.id)
