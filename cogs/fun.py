...
class Fun(commands.Cog):
    def __init__(self, client):
        self.client = client
        self.flavors = [
        'Strawberry Ice', 'Blueberry Blast', 'Mint Chill', 'Vanilla Custard', 'Mango Tango',
        'Watermelon Breeze', 'Grape Escape', 'Apple Frost', 'Lemon Zest', 'Peach Fizz',
        'Cherry Bomb', 'Coconut Dream', 'Berry Burst', 'Pineapple Punch', 'Cola Fizz',
        'Raspberry Rush', 'Banana Split', 'Tropical Twist', 'Candy Crush', 'Bubblegum Pop',
        'Chocolate', 'Cinnamon', 'Orange', 'Pear', 'Kiwi',
        'Pomegranate', 'Honeydew', 'Blackberry', 'Lychee', 'Green Tea',
        'Caramel Swirl', 'Maple Syrup', 'Cotton Candy', 'Mojito Mint', 'Vanilla Bean',
        'Lime Tart', 'Sour Apple', 'Fruit Punch', 'Toasted Marshmallow', 'Peppermint Patty',
        'Peanut Butter Cup', 'Tiramisu', 'Mango Sticky Rice', 'Dragon Fruit', 'Cactus Cooler',
        'Cinnamon Roll', 'Almond Cookie', 'Passion Fruit', 'Chai Tea', 'Milkshake',
        'Fruity Pebbles', 'Lemonade Stand', 'Ginger Snap', 'Spiced Pumpkin', 'Coffee Delight',
        'Brown Sugar Cinnamon', 'Nutty Hazelnut', 'Vanilla Toffee', 'Raspberry Cheesecake', 'Chocolate Mint',
        'Banana Cream Pie', 'Pistachio Ice Cream', 'Peanut Butter Banana', 'Cookie Dough Bliss', 'Strawberry Shortcake',
        'Apple Pie à la Mode', 'Smores Delight', 'Caramel Macchiato', 'Vanilla Pudding', 'Blueberry Muffin',
        'Cherry Almond Tart', 'Tropical Coconut Cream', 'Fudge Brownie', 'Chocolate Raspberry Swirl', 'Lemon Meringue Pie', 'Vanilla'
    ]
    ...
    async def hit(self, ctx):
        async with self.client.pool.acquire() as conn:
            result = await conn.fetchrow("SELECT flavor, hits FROM vape WHERE user_id = $1", ctx.author.id)
        
        if not result or not result['flavor']:
            await ctx.deny("You haven't set a flavor, use `-vape flavor <flavor>` to set one.")
            return
        ...
        async with self.client.pool.acquire() as conn:
            await conn.execute("UPDATE vape SET hits = $1 WHERE user_id = $2 AND flavor = $3", hits, ctx.author.id, flavor)

        embed = discord.Embed(description=f"> <:vape:1296191531241312326> {ctx.author.mention}: You hit the flavor **{flavor},** now have `{hits}` hits", color=color.default)
    ...
    async def flavors(self, ctx):
        ...
        for page in pages:
            ...
            embed = discord.Embed(title="Available Vape Flavors", description=flavor_list, color=color.default)
            embeds.append(embed)
    ...
    async def vape_steal(self, ctx: commands.Context, member: discord.Member):
        ...
        async with self.client.pool.acquire() as conn:
            result = await conn.fetchrow("SELECT flavor FROM vape WHERE user_id = $1", member.id)
            ...
            await conn.execute(
                "INSERT INTO vape (user_id, flavor, hits) VALUES ($1, $2, 0) ON CONFLICT (user_id) DO UPDATE SET flavor = $2",
                ctx.author.id, stolen_flavor)
            await conn.execute("UPDATE vape SET flavor = NULL WHERE user_id = $1", member.id)
        ...
        embed = discord.Embed(description=f"> {ctx.author.mention} has stolen **{stolen_flavor}** from {member.mention}!", color=discord.Color.blue())
        await ctx.send(embed=embed)
    ...
    async def vape_hits(self, ctx: Context):
        ...
        async with self.client.pool.acquire() as conn:
            results = await conn.fetch("SELECT user_id, hits FROM vape WHERE hits > 0 ORDER BY hits DESC")
        
        if not results:
            await ctx.deny("No hits recorded in the server.")
            return
        
        hit_list = "\n".join([f"<@{row['user_id']}>: `{row['hits']}` hits" for row in results])
        
        embed = discord.Embed(title="Vape hits in the Server", description=hit_list, color=color.default)
        await ctx.send(embed=embed)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get("https://official-joke-api.appspot.com/jokes/random") as resp:
                if resp.status == 200:
                    ...
                    embed = discord.Embed(description=f"> **Here's a joke for you:** `{joke}`", color=color.default)
                    await ctx.send(embed=embed)
                else:
    ...
    async def coinflip(self, ctx: Context):
        ...
        embed = discord.Embed(description=f"The coin landed on **{result}!**", color=color.default)
        await ctx.send(embed=embed)
    ...
    async def hug(self, ctx: Context, member: discord.Member):
        ...
        embed = discord.Embed(description=f"**{ctx.author.mention}** gives a warm hug to **{member.mention}!** 🤗", color=color.default)
    ...
    async def roast(self, ctx: Context, member: discord.Member):
        ...
        embed = discord.Embed(description=f"> **{ctx.author.mention}** roasts **{member.mention}**: `{roast}`", color=color.default)
        await ctx.send(embed=embed)
    ...
    async def compliment(self, ctx: Context, member: discord.Member):
        ...
        embed = discord.Embed(description=f"**{ctx.author.mention}** compliments **{member.mention}:** {compliment}", color=color.default)
        await ctx.send(embed=embed)
    ...
    async def slap(self, ctx: Context, member: discord.Member):
        ...
        embed = discord.Embed(description=f"**{ctx.author.mention}** slaps **{member.mention}!** 👋", color=color.default)
        embed.set_image(url=image_url)
        await ctx.send(embed=embed)
    ...
    async def highfive(self, ctx: Context, member: discord.Member):
        ...
        embed = discord.Embed(description=f"**{ctx.author.mention}** gives **{member.mention}** a high five! ✋", color=color.default)
        embed.set_image(url=image_url)
        await ctx.send(embed=embed)

    @commands.command(
        description="Start a dance party", 
        aliases=["d"]
    )
    async def dance(self, ctx: Context):
        async with aiohttp.ClientSession() as session:
            api_key = "PhdRFX2usNqyZo8KLmqOjAOHOSzpMuco"
            async with session.get(f"https://api.giphy.com/v1/gifs/random?api_key={api_key}&tag=dance") as resp:
                if resp.status == 200:
                    data = await resp.json()
                    gif_url = data["data"]["images"]["original"]["url"]
                    embed = discord.Embed(description=f"**{ctx.author.mention}** starts a dance party! 💃🕺", color=color.default)
    ...
    async def pet(self, ctx: Context, member: discord.Member):
        if member.id == ctx.author.id:
            ...
            return
        embed = discord.Embed(description=f"**{ctx.author.mention}** pets **{member.mention}!** 🐾", color=color.default)
        await ctx.send(embed=embed)
    ...
    @truthordare.command(
        description="Choose a truth question"
    )
    async def truth(self, ctx: Context):
        ...
        embed = discord.Embed(description=f"**Truth:** {truth_question}", color=color.default)
        await ctx.send(embed=embed)
    ...
    async def dare(self, ctx: Context):
        ...
        embed = discord.Embed(description=f"**Dare:** {dare_challenge}", color=color.default)
        await ctx.send(embed=embed)
