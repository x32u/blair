...
class Information(commands.Cog):
    ...
    async def ping(self, ctx: commands.Context):
        ...
        await ctx.send(embed=discord.Embed(
            description=f"> :stars: **Ping:** `{latency} ms`",
            color=color.default))
    ...
    async def uptime(self, ctx: commands.Context):
        ...
        hours, minutes, seconds = uptime_delta.seconds // 3600, (uptime_delta.seconds // 60) % 60, uptime_delta.seconds % 60
    ...
    async def support(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            ...
            color=color.default)
        view = View().add_item(Button(label="Join Support", url="https://discord.gg/blare"))
    ...
    async def botinfo(self, ctx):
        ...
        total_commands = sum(
            ...
            for command in self.client.commands if not command.hidden and command.cog_name != "Jishaku")

        view = View()
        view.add_item(Button(style=discord.ButtonStyle.link, label="Support", url="https://discord.gg/blare"))
        view.add_item(Button(style=discord.ButtonStyle.link, label="Invite me", url="https://discordapp.com/oauth2/authorize?client_id=1161529597012226099&scope=bot+applications.commands&permissions=8"))
    ...
    async def guildicon(self, ctx: commands.Context, guild_id: int = None):
        ...
        if icon_url:
            await ctx.send(embed=discord.Embed(
                ...
                color=color.default
            ).set_image(url=icon_url).set_footer(
    ...
    async def guildbanner(self, ctx: commands.Context, guild_id: int = None):
        ...
        if banner_url:
            await ctx.send(embed=discord.Embed(
                ...
                color=color.default
            ).set_image(url=banner_url).set_footer(
                text=f"Requested by {ctx.author.display_name}",
                icon_url=ctx.author.display_avatar.url
            ))
        else:
    ...
    async def splash(self, ctx: commands.Context, guild_id: int = None):
        ...
        if guild.splash:
            await ctx.send(
                embed=discord.Embed(
                    title=f"{guild.name}'s Splash Background",
                    color=color.default
                ).set_image(url=guild.splash.url).set_footer(
                    text=f"Requested by {ctx.author.display_name}",
                    icon_url=ctx.author.display_avatar.url)
            )
        else:
    ...
    async def inviteinfo(self, ctx: commands.Context, invite_code: str):
        ...
        try:
            ...
            embed.add_field(name="Expiration", value=invite.expires_at.strftime('%Y-%m-%d %H:%M:%S') if invite.expires_at else "Never", inline=False)
            embed.set_footer(text=f"Requested by {ctx.author.display_name}", icon_url=ctx.author.display_avatar.url)
    ...
    async def oldest(self, ctx: Context):
        ...
        try:
            ...
            if youngest_member:
                ...
                await ctx.deny("Could not retrieve the member with the most recent account creation date.")
        except Exception as e:
    ...
    async def youngest(self, ctx: Context):
        ...
        try:
            ...
            if oldest_member:
                ...
                await ctx.deny("Could not retrieve the member with the oldest account creation date.")
        except Exception as e:
            await ctx.deny(f"An error occurred: {str(e)}")
    ...
    async def inrole(self, ctx: commands.Context, role: discord.Role):
        ...
        embed = discord.Embed(
            ...
            color=color.default
        ).set_footer(text=f"Total Members: {len(members_with_role)}", icon_url=ctx.guild.icon.url)
        await ctx.send(embed=embed)
    ...
    async def boosters(self, ctx: commands.Context):
        ...
        embed.description = (
            ", ".join(boosters) if boosters else "> There are no boosters in this server.")
        await ctx.send(embed=embed)
    ...
    async def boosters_lost(self, ctx: commands.Context):
        ...
        lost_boosters = [
            ...
            if member.premium_since is None and hasattr(member, 'premium') and member.premium]
        response = (
            ", ".join(lost_boosters) if lost_boosters 
    ...
    @discord.app_commands.allowed_installs(guilds=True, users=True)
    @discord.app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    async def banner(self, ctx: Context, member: Union[discord.Member, discord.User] = None):
        ...
        if user.banner:
            embed = discord.Embed(
                ...
                color=color.default
            ).set_image(url=user.banner.url).set_footer(
                text=f"Requested by {ctx.author.display_name}",
                icon_url=ctx.author.display_avatar.url
            )
        ...
        try:
            async for msg in channel.history(limit=1, oldest_first=True):
                embed = discord.Embed(
                    ...
                    color=color.default
                ).set_footer(text=f"Sent by {msg.author} on {msg.created_at.strftime('%Y-%m-%d %H:%M:%S')}")
                view = discord.ui.View()
                view.add_item(discord.ui.Button(
    ...
    async def userinfo(self, ctx: commands.Context, member: discord.Member = None):
        ...
        embed = discord.Embed(
            ...
            color=color.default)
        created_at = format_dt(member.created_at, style='D') if hasattr(member, 'created_at') else 'n/a'
        if isinstance(member, discord.Member):
            joined_at = format_dt(member.joined_at, style='D') if member.joined_at else 'n/a'
