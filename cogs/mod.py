...
class Moderation(commands.Cog):
    ...
    async def kick(self, ctx: commands.Context, member: discord.Member, *, reason: str = "None"):
        ...
        await self.send_agree(ctx, f"**Kicked** {member.mention} with reason: `{reason}`")
    ...
    async def ban(self, ctx: commands.Context, member: discord.Member, *, reason: str = "None"):
        ...
        await self.send_agree(ctx, f"**Banned** {member.mention} with reason: `{reason}`")
    ...
    async def mute(self, ctx: Context, member: discord.Member, duration: str = '5m'):
        if member.top_role >= ctx.author.top_role:
            return await ctx.deny("You **cannot** mute someone with an equal or higher role")
        ...
        await self.send_agree(ctx, f"**Muted** {member.mention} for {time_value} {unit}.")
    ...
    @commands.has_permissions(manage_channels=True)
    async def channelattachments(self, ctx: commands.Context, channel: discord.TextChannel = None, *, action: str):
        ...
        asyncio.create_task(channel.set_permissions(ctx.guild.default_role, overwrite=overwrite))
        await ctx.agree(f"**Attachment permissions turned {action}** for {channel.mention}")
    ...
    @commands.has_permissions(manage_channels=True)
    async def channelpermit(self, ctx: commands.Context, member: discord.Member, channel: discord.TextChannel):
    ...
    @commands.has_permissions(manage_channels=True)
    async def channelprivate(self, ctx: commands.Context, channel: discord.TextChannel = None, *, action: str):
    ...
    @commands.has_permissions(manage_channels=True)
    async def channelrename(self, ctx: commands.Context, channel: discord.TextChannel, *, new_name: str):
    ...
    @commands.has_permissions(manage_channels=True)
    async def channelrevoke(self, ctx: commands.Context, member: discord.Member, channel: discord.TextChannel):
        """Revoke the specified member's permission to view the specified channel."""
        overwrite = channel.overwrites_for(member)
        overwrite.read_messages = False

        asyncio.create_task(channel.set_permissions(member, overwrite=overwrite))
        await ctx.agree(f"**Revoked** {member.mention}'s permission to view {channel.mention}")
    ...
    async def lockdown(self, ctx: commands.Context):
        ...
        async def lock_channel():
            ...
            await ctx.agree(f"**Lockdown activated in** {ctx.channel.mention}. Members cannot send messages.")
    ...
    async def lockdown_all(self, ctx: commands.Context):
        ...
        tasks = [
            ...
            for channel in ctx.guild.channels if isinstance(channel, discord.TextChannel)
        ]

        async def lock_all_channels():
            await asyncio.gather(*tasks)
            await ctx.agree("**Lockdown activated in all channels.** Members cannot send messages.")
    ...
    async def unlockdown_all(self, ctx: commands.Context):
        ...
        async def unlock_all_channels():
            await asyncio.gather(*tasks)
            await ctx.agree("**Lockdown deactivated in all channels.** Members can send messages.")
