        ...
        if custom_id in ["lock", "unlock", "hide", "reveal", "rename", "decrease", "increase", "info", "kick", "delete"]:
            ...
            if user_channel and user_channel.category == category and user_channel != create_channel:
                ...
                if owner_id != interaction.user.id and custom_id != "delete":
                    await interaction.response.send_message(embed=discord.Embed(description=f"> {emoji.deny} {interaction.user.mention}: You **aren't** the owner of this voice channel", color=color.deny), ephemeral=True)
                    return
                ...
                await interaction.response.send_message(embed=discord.Embed(description=f"> {emoji.deny} {interaction.user.mention}: You **aren't** in a voice channel", color=color.deny), ephemeral=True)
